...
overview: 创建两个独立的 Demo 项目进行组件库对比：Vue 3 + TDesign 方案 和 React + shadcn/ui 方案。各自展示完整的功能页面（包括按钮、表单、表格、卡片、对话框等组件），让用户在同一浏览器中对比两种技术栈的实际效果。
design:
  architecture:
    framework: react
    component: shadcn
  ...
  colorSystem:
    ...
    functional:
      ...
      - "#E2E8F0"
todos:
  - id: scaffold-react-shadcn
    content: 使用 [skill:prototype] 创建 frontend-demo/react-shadcn/：Vite + React 18 + TypeScript 项目骨架，安装 Tailwind CSS 3、shadcn 底层依赖，配置 CSS 变量主题
    status: completed
  - id: setup-shadcn-components
    content: 创建 src/lib/utils.ts 的 cn 函数，手动复制 14 个 shadcn/ui 组件源码到 src/components/ui/
    status: completed
    dependencies:
      - scaffold-react-shadcn
  - id: build-react-page
    content: 实现 react-shadcn 完整展示页面：顶部导航 + 左侧锚点菜单 + 五个组件分区（按钮/表单/数据展示/导航/反馈）
    status: completed
    dependencies:
      - setup-shadcn-components
  - id: scaffold-vue-tdesign
    content: 使用 [skill:prototype] 创建 frontend-demo/vue-tdesign/：Vite + Vue 3 + TypeScript 项目骨架，安装 TDesign Vue Next
    status: completed
  - id: build-vue-page
    content: 实现 vue-tdesign 完整展示页面：顶部导航 + 左侧锚点菜单 + 五个组件分区（按钮/表单/数据展示/导航/反馈），与 react-shadcn 场景一致
    status: completed
    dependencies:
      - scaffold-vue-tdesign
---
