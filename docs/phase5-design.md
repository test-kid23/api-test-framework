...
```
framework/execution_orchestrator.py          # 🆕 执行编排器
worker/tasks.py                              # ♻️ 重构为调用编排器
api/routers/executions.py                     # ♻️ 重构为调用编排器
tests/framework/test_execution_orchestrator.py # 🆕 单元测试
```
...
if TYPE_CHECKING:
    ...
    from framework.persistence.repositories.execution_repository import ExecutionRepository
    from framework.persistence.repositories.report_repository import ReportRepository
...
class ExecutionOrchestrator:
    ...
    async def execute_single_case(
        self, case: TestCase, variables: dict[str, object] | None = None
    ) -> ExecutionResult:
        """执行单个用例.
    ...
    async def execute_case_list(
        self, cases: list[TestCase], variables: dict[str, object] | None = None
    ) -> ExecutionResult:
        """批量执行用例列表.
    ...
    async def execute_suite(self, suite_id: str) -> ExecutionResult:
        """执行测试套件.
...
def run_execution(self, execution_id: str) -> dict[str, object]:
    ctx = _build_execution_context(execution_id)
    orchestrator = ExecutionOrchestrator(ctx)
    result = asyncio.run(orchestrator.execute_single_case(ctx.case))
    return asdict(result)
```

**验收标准**：

- [ ] `ExecutionOrchestrator` 三个方法覆盖所有执行场景
- [ ] Worker 和 API 路由均通过编排器执行，无直接调用 runner
- [ ] 单元测试覆盖率 ≥ 95%（mock runner 和 repos）
- [ ] 已有集成测试全部通过
...
```
framework/context_snapshot.py                   # 🆕 快照生成逻辑
framework/persistence/models/context_snapshot.py # 🆕 ORM 模型
alembic/versions/xxxx_add_context_snapshots.py   # 🆕 迁移
api/routers/executions.py                        # ♻️ 新增查询接口
tests/framework/test_context_snapshot.py         # 🆕 单元测试
```
...
class ContextSnapshot:
    ...
    Attributes:
        ...
        step_index: 失败步骤索引
        run_vars: 运行级变量（MappingProxyType 保证不可变）
        case_vars: 用例级变量
        step_vars: 步骤级变量
        error_message: 失败信息
        traceback: 完整堆栈
...
class ContextSnapshotManager:
    ...
    async def capture_on_failure(
        self, execution_id: str, step_index: int, ctx: ExecutionContext, error: Exception
    ) -> ContextSnapshot:
        """失败时捕获快照.
    ...
    async def get_snapshot(self, execution_id: str) -> ContextSnapshot | None:
        """查询快照.
...
```python
# framework/persistence/models/context_snapshot.py
...
class ContextSnapshotModel(Base):
    ...
    execution_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("executions.id"), index=True, nullable=False
    )
    step_index: Mapped[int] = mapped_column(Integer, nullable=False)
    run_vars: Mapped[dict] = mapped_column(JSON, nullable=False, default=dict)
    case_vars: Mapped[dict] = mapped_column(JSON, nullable=False, default=dict)
    step_vars: Mapped[dict] = mapped_column(JSON, nullable=False, default=dict)
...
```
GET /api/v1/executions/{execution_id}/snapshot → ContextSnapshotResponse
```

**验收标准**：

- [ ] 执行失败后 DB 中有对应 snapshot 记录
- [ ] `ContextSnapshot` 使用 `frozen=True` + `MappingProxyType` 保证不可变
- [ ] API 查询接口可正常返回快照数据
- [ ] 快照不包含敏感字段（token/password 已脱敏）
- [ ] 单元测试覆盖正常/异常/空变量三种场景
...
```
framework/scheduler.py              # ♻️ 新增失败告警逻辑
tests/framework/test_scheduler.py   # ♻️ 新增告警测试用例
```

**改动要点**：

```python
# framework/scheduler.py 调度任务包装
async def _execute_scheduled_job(schedule_id: str) -> None:
    try:
        await orchestrator.execute_suite(schedule.suite_id)
    except Exception as e:
        logger.error("schedule_execution_failed",
        ...
        await notification_service.send_alert(
            AlertRule(
                name="调度执行失败",
                level="critical",
                message=f"定时任务 {schedule.name} 执行失败: {e}",
...
- [ ] 异常类型有区分（环境不可用 / Worker 离线 / 用例语法错误）
- [ ] 单元测试 mock NotificationService 验证调用
...
| 编号 | 任务 | 优先级 | 工时 | 状态 | 完成日期 |
|------|------|--------|------|------|---------|
| T5-01 | 执行编排统一 | P0 | 3d | ⬜ | — |
| T5-02 | 上下文快照持久化 | P0 | 2d | ⬜ | — |
| T5-03 | Worker 健康监控 | P0 | 2d | ⬜ | — |
| T5-04 | 调度失败告警 | P0 | 1d | ⬜ | — |
| T5-05 | 组合断言 (AND/OR) | P1 | 3d | ⬜ | — |
| T5-06 | 提取器管道 | P1 | 2d | ⬜ | — |
| T5-07 | 插件配置化 | P1 | 1d | ⬜ | — |
| T5-08 | 多数据源注册 | P1 | 2d | ⬜ | — |
| T5-09 | Email 通知完善 | P1 | 2d | ⬜ | — |
| T5-10 | 环境变量加密 | P1 | 2d | ⬜ | — |
| T5-11 | 报告聚合 API | P1 | 3d | ⬜ | — |
| T5-12 | 快照 Redis 缓存 | P1 | 1d | ⬜ | — |
| T5-13 | 配置热加载 | P2 | 1d | ⬜ | — |
...
**Phase 5a 验收（第 2 周末）**：
- [ ] 执行编排统一后 Worker 和 API 路由无重复执行逻辑
- [ ] 执行失败可回溯快照
- [ ] Worker 失联 60s 内触发告警
- [ ] 调度失败触发告警通知
...
*文档结束 · Phase 5 详设 v1.0 · 2026-06-11*
