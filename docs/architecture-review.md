...
| WebSocket 模块 | 6.5/10 | WsStepExecutor 策略化，⚠️ 同步适配仍为 Hack 式（结构性隐患，详见 §10.1 #2） |
...
### 10.1 结构性隐患（必须在 Phase 2 开工前专项处理）

以下两个问题不是普通的"功能缺失"，而是**架构级结构性缺陷**。如果不提前解决，将在后续阶段放大为系统性风险：

| # | 隐患 | 风险分析 | 建议 | 目标阶段 |
|---|------|---------|------|---------|
| 1 | 用例整体无超时管控 | 当前 HTTP 单请求有超时，但一个 YAML 用例（setup + 请求 + extract + teardown）整体执行无上限。平台化后，一个失控用例可长期占用 Worker，拖垮调度队列。这不是"增强"而是执行引擎的**基础安全能力** | Phase 2 开工第一优先：引入 runner 级用例超时（默认 300s），超时标记 `TIMEOUT` 并释放资源。建议同时支持 pytest-timeout 作为外部兜底 | **Phase 2 首位** |
| 2 | WebSocket 同步适配 Hack 式 | `WsStepExecutor` 虽已策略化，但底层同步适配仍依赖 `nest_asyncio` patch 或线程池桥接。Phase 3 上 Celery 时，Worker 内事件循环冲突将导致难以排查的挂起/死锁。**必须在上分布式之前彻底解决** | Phase 2 将 WS 执行路径迁移到原生 asyncio：`AsyncWsStepExecutor` + `httpx.WebSocket` 原生异步。移除 `nest_asyncio` 依赖，确保所有执行路径在同一事件循环内 | **Phase 2** |

### 10.2 紧急（P0）

| # | 问题 | 建议 | 目标阶段 |
|---|------|------|---------|
| 1 | 执行失败无上下文快照 | 失败时自动调用 context.snapshot() 持久化 | Phase 2 |
...
| # | 问题 | 建议 | 目标阶段 |
|---|------|------|---------|
| 1 | 配置热加载未实现 | watchdog 文件监听 + 重载 | Phase 2 |
| 2 | 超时不支持单接口覆盖 | HttpRequest.timeout 字段 | Phase 2 |
| 3 | 缺少签名计算函数 | HMAC-SHA256 / RSA 内置到模板 | Phase 2 |
...
> **总结**: 经过 Phase 0a/0b/1 三个阶段的重构，框架在**引擎层**的架构质量（策略模式、拦截器链、插件系统、结构化日志、协程安全上下文、安全加固）已达到大厂 API 测试框架标准。下一阶段（Phase 2）在推进服务化之前，**必须优先处理两个结构性隐患**（用例超时管控 + WebSocket asyncio 原生迁移），它们是执行引擎的基础安全能力和架构级缺陷，推迟将导致分布式阶段产生系统性风险。隐患消除后，核心目标是**引擎服务化与持久化**，将框架从"单体引擎"推向"可服务化的平台基石"。
